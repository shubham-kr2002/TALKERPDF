    ...
    for img_index, img in enumerate(image_list):
        ...
        try:
            ...
            if is_logo_or_icon(w, h):
                print(f"    ⊗ Skipping small image {w}x{h} (Logo/Icon)")
                continue  # Skip this image -> Save API cost!
            
            print(f"    ✓ Valid chart found: {w}x{h} pixels")
...
def process_vision_task(task_data):
    ...
    try:
        ...
        print(f"      ✗ Thread error processing page {page_num + 1}, image {img_idx + 1}: {e}")
        return {
            'page': page_num,
            'img_idx': img_idx,
            'description': "",
...
def ingest_pdfs(uploaded_files):
    ...
    for uploaded_file in uploaded_files:
        ...
        for page_num, page_text in pages_text:
            ...
            if valid_images:
                print(f"    → Queued {len(valid_images)} charts for parallel processing")
        ...
        doc.close()
        print(f"\n  ✓ Extraction complete: {len(page_data)} pages, {len(pending_tasks)} images queued")
        ...
        if pending_tasks:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                ...
                for future in as_completed(future_to_task):
                    ...
                    # Store result indexed by (page, img_idx)
                    key = (result['page'], result['img_idx'])
                    vision_results[key] = result
                    
                    if result['success']:
                        print(f"  [{completed}/{len(pending_tasks)}] ✓ Page {result['page'] + 1}, Chart {result['img_idx'] + 1}: {len(result['description'])} chars")
                    else:
                        print(f"  [{completed}/{len(pending_tasks)}] ✗ Page {result['page'] + 1}, Chart {result['img_idx'] + 1}: Failed")
            
            print(f"\n  ✓ Parallel processing complete: {sum(1 for r in vision_results.values() if r['success'])}/{len(pending_tasks)} successful")
        else:
            print(f"\n  → No images to process (text-only document)")
            vision_results = {}
        
        # ============================================================
        # PHASE 3: ASSEMBLY (Combine text + vision descriptions)
        # ============================================================
        print(f"\n{'='*60}")
        print(f"PHASE 3: Assembling chunks with visual descriptions")
        print(f"{'='*60}")
        ...
        for page_info in page_data:
            ...
            if combined_content.strip():
                ...
                for chunk_idx, chunk in enumerate(chunks):
                    ...
                    # ChromaDB requires string/int/float/bool, so convert list to JSON string
                    if image_paths:
        ...
        print(f"\n  ✓ Assembly complete: {len(all_chunks)} total chunks")
        ...
        print(f"{'='*60}")
        print(f"  📄 Text-only chunks: {text_chunks}")
        print(f"  🖼️  Chunks with visuals: {visual_chunks}")
